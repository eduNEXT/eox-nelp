...
def create_external_certificate(external_certificate_data, user_id=None):
...
@shared_task
def trigger_external_certificate_sqs(external_certificate_data, user_id):
    ...
    if sqs_response:
        logger.info(
            "External certificate triggered with  MessageId %s created successfully for user_id %s.",
            sqs_response.get("MessageId"),
            user_id,
        )
    else:
        logger.error(
            "Failed to trigger external certificate for user_id %s. Response: %s",
            user_id,
            sqs_response,
        )
